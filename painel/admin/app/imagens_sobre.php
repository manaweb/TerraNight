<? 
	...
	if ($_GET['faz']=="dados") {
		...
		$dados2 = array('imagem' => array());
		for($i = 0; $i < $size; $i++){
			if (!empty($_FILES['imagem'][$i]['name'])) {
				$dados2['imagem'][$i] = processaArquivo('imagem',$Config,$_FILES,1,'imagem');
				if ($dados['imagem'][$i] == false) { header("Location: ../sys/".$Config2['arquivo'].".php?erro=".urlencode('Erro processando Imagem.'),true); exit; }
			}	
		}
		...
		if ($$Config['id']>0) {
			...
			for($i = 0; $i < $size; $i++){
				if (strlen($dados["imagem"][$i])>0) {

					db_apagaArquivo(,$Config,$$Config['id']);
				}
			}
