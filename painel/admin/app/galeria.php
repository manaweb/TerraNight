	...
	if ($_GET['faz']=="fotosForm") {
		...
		for ($num=1;$num<=8;$num++) {
			if (strlen($_FILES['imagem'.$num]['name']) > 4) {
				...
				if ($Arquivo != false) {
					if (Miniatura("../../arquivos/tmp/".$Arquivo, "../../arquivos/galeria/".$codigo."/miniaturas/".$novoArquivo, 360, 240, 1, 0)) {

						if (Miniatura("../../arquivos/tmp/".$Arquivo, "../../arquivos/galeria/".$codigo."/fotos/".$novoArquivo, 1920, 1080, 0, 1, $ArqMarcadagua, GaleriaConfigValor('marcadagua_distancia'),GaleriaConfigValor('marcadagua_posicao'),GaleriaConfigValor('marcadagua_opacidade'))) {
